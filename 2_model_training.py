...
print(f"加载的数据集: X_test {X_test.shape}, y_test {y_test.shape}")
...
# 训练决策树模型
print("\n训练决策树模型...")
dt = DecisionTreeClassifier(random_state=42, max_depth=10)  # 简化版本，不使用网格搜索
dt.fit(X_train, y_train)
dt_results = evaluate_model(dt, X_test, y_test, 'DecisionTree')
...
# 训练随机森林模型
print("\n训练随机森林模型...")
rf = RandomForestClassifier(random_state=42, n_estimators=100, max_depth=10)  # 简化版本
rf.fit(X_train, y_train)
rf_results = evaluate_model(rf, X_test, y_test, 'RandomForest')
